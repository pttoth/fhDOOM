		...
		if ( alIsSource( chan->openalSource ) ) {
			...
			if (idSoundSystemLocal::s_useEAXReverb.GetBool()) {
				if (enviroSuitActive) {
					...
					alSource3i(chan->openalSource, AL_AUXILIARY_SEND_FILTER, alListenerEffectSlot, 0, alListenerFilter);
				}
				else {
					alSource3i(chan->openalSource, AL_AUXILIARY_SEND_FILTER, alListenerEffectSlot, 0, AL_FILTER_NULL);
				}
			}
