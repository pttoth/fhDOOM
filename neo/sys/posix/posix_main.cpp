...
=================
*/
sysDllHandle_t Sys_DLL_Load( const char *path ) {
	sysDllHandle_t handle;
	handle.value = dlopen( path, RTLD_NOW );
	if ( !handle ) {
...
=================
*/
void* Sys_DLL_GetProcAddress( sysDllHandle_t handle, const char *sym ) {
	const char *error;
	void *ret = dlsym( handle.value, sym );
...
=================
*/
void Sys_DLL_Unload( sysDllHandle_t handle ) {
	dlclose( handle.value );
}
