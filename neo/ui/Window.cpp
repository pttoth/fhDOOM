...
#if 1
	ptrdiff_t ret = -1;
	const auto _this = (ptrdiff_t)this;

	if ( wv == &rect ) {
		ret = (ptrdiff_t)&this->rect - _this;
	}

	if ( wv == &backColor ) {
		ret = (ptrdiff_t)&this->backColor - _this;
	}

	if ( wv == &matColor ) {
		ret = (ptrdiff_t)&this->matColor - _this;
	}

	if ( wv == &foreColor ) {
		ret = (ptrdiff_t)&this->foreColor - _this;
	}

	if ( wv == &borderColor ) {
		ret = (ptrdiff_t)&this->borderColor - _this;
	}

	if ( wv == &textScale ) {
		ret = (ptrdiff_t)&this->textScale - _this;
	}

	if ( wv == &rotate ) {
		ret = (ptrdiff_t)&this->rotate - _this;
	}
...
void idWindow::FixupTransitions() {
	...
	for ( i = 0; i < c; i++ ) {
		...
		if ( dw && ( dw->win || dw->simp ) ){
			if ( dw->win ) {
				if ( transitions[i].offset == (ptrdiff_t)&this->rect - _this ) {
					transitions[i].data = &dw->win->rect;
				} else if ( transitions[i].offset == (ptrdiff_t)&this->backColor - _this) {
					transitions[i].data = &dw->win->backColor;
				} else if ( transitions[i].offset == (ptrdiff_t)&this->matColor - _this ) {
					transitions[i].data = &dw->win->matColor;
				} else if ( transitions[i].offset == (ptrdiff_t)&this->foreColor - _this ) {
					transitions[i].data = &dw->win->foreColor;
				} else if ( transitions[i].offset == (ptrdiff_t)&this->borderColor - _this ) {
					transitions[i].data = &dw->win->borderColor;
				} else if ( transitions[i].offset == (ptrdiff_t)&this->textScale - _this ) {
					transitions[i].data = &dw->win->textScale;
				} else if ( transitions[i].offset == (ptrdiff_t)&this->rotate - _this ) {
